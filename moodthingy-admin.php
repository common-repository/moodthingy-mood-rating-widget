...
class MoodThingy_Widget extends WP_Widget {
	function MoodThingy_Widget() {
		...
		$widget_ops = array('classname' => 'moodthingy_widget', 'description' => 'The most popular posts from your MoodThingy widget' );
		$this->WP_Widget('moodthingy_widget', 'MoodThingy', $widget_ops);		
	}
		...
		<p>
			<label for="<?php echo $this->get_field_id('duration'); ?>">
				...
				<?php for ($i=1; $i<=6; $i=$i+1) { ?>
					<option value="MOOD-<?php echo $i; ?>"<?php echo ($duration == 'MOOD-' . $i ) ? ' selected="selected"' : ''; ?>>posts voted most "<?php echo $moods[$i]; ?>"</option>
				<?php } ?>
				</select>
			</label>
		</p>
	...
	<div class="wrap">
		...
		<div class="container">	
			...
			<div class="content" id="customize">
				<h3></h3>
				<img src="<?php echo MOODTHINGY_PLUGIN_DIR . '/moodthingy-cusomization.jpg'; ?>" style="float:right; border:5px solid #eee; margin-left:20px; width:300px;">
				<p><big>The ability to customize colors and moods on MoodThingy, as well as the ability to have less than six moods, is available by <a href="http://codecanyon.net/item/moodthingy-mood-rating-widget-for-wordpress-pro/2255966" target="_blank">purchasing the MoodThingy Mood Rating Widget for WordPress PRO plug-in on CodeCanyon</a>.</big></p> 
				
				<p>You can easily remove the MoodThingy logo in the PRO version as well. Support for the PRO version of MoodThingy helps keeps us developing both the free and the Pro versions!</p>
				...
				<div id="csswrapper">
					...
					<p class="description">Uncomfortable with CSS?  If you're just looking to change the colors, <a href="http://codecanyon.net/item/moodthingy-mood-rating-widget-for-wordpress-pro/2255966" target="_blank">consider purchasing the PRO version</a>!</p>
				</div>
			...
			<div class="content" id="faq">
				...
				<p>If your WordPress theme uses widgets, a MoodThingy Widget is available in the Widgets menu which can list the most voted on posts, as well as posts voted most Fascinated, Amused, Sad, Angry, Bored and Excited.</p>
